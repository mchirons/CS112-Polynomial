...
}$PBJQ(".tab-pane").css("max-height",a);
dhtml_view_sites()
};
...
function publishSite(c){var a="/direct/site/"+c+"/edit";
var b=$("input[name='sakai_csrf_token']").val();
var d=$PBJQ.ajax({type:"POST",data:"published=true&csrftoken="+b,url:a,success:function(){location.reload()
}}).responseText
...
var collapsed=false;
var $window=$PBJQ(window),$tools=$("#toolMenu"),padding=$(".Mrphs-siteHierarchy").height()+$(".Mrphs-topHeader").height();
$PBJQ(document).ready(function(){if(getCookieVal("sakai_nav_minimized")==="true"){$PBJQ(".js-toggle-nav").click();
collapsed=true
}});
function animateToolBar(){var b=$(".pasystem-banner-alerts").height();
var a=($window.height()-(padding+b))>$tools.height();
if($("#toolMenuWrap").css("position")!=="fixed"&&a&&$window.scrollTop()>0){$("#toolMenu").stop().animate({top:$window.scrollTop()+b})
...
$PBJQ(window).scroll(function(){if(currentHeaderWidth>799){var b=0;
...
$(header).stop().animate({height:$(".is-fixed").css("height")},200)
}}else{$(".Mrphs-mainHeader").removeClass("is-fixed");
$(header).data("sticked",false);
$(header).css("height",null);
$(header).stop().animate({height:mainHeaderSize},200)
}animateToolBar()
}else{$(".Mrphs-mainHeader").removeClass("is-fixed")
}});
